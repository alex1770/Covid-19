...
for (pl,va) in [(p0,v0), (p1,v1), (p0+p1,v0+v1)]:
  ...
  N=1000000
  Lam=gamma.rvs(pl+alpha-1, scale=1/(1+beta), size=N)
  Mu= gamma.rvs(va+alpha-1, scale=1/(1+beta), size=N)
  
  l=[mu/lam for (lam,mu) in zip(Lam,Mu)]
  l.sort()

  print("Number of infected participants on placebo arm:",pl)
  print("Number of infected participants on treatment arm:",va)
  print("Using prior of Gamma(shape=%f, scale=%f)"%(alpha,1/beta))
  for c in [0.5, 0.9, 0.95, 0.98, 0.99]:
