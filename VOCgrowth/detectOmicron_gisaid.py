...
print("Using data from",datafile,file=sys.stderr)
print("Dates",mindate,"-",maxdate,file=sys.stderr)
print("Using threshold",thr,file=sys.stderr)

mutcounts={}
n=0
with open(datafile,'r') as fp:
  for (dt,mut) in csvrows_it(fp,['Collection date','AA Substitutions'],sep='\t'):
    if dt<mindate or dt>maxdate: continue
    muts=mut[1:-1].split(',')
    for mut in muts:
...
l=[]
with open(datafile,'r') as fp:
  for (name,dt,lin,mut,row) in csvrows_it(fp,['Virus name','Collection date','Pango lineage','AA Substitutions',None],sep='\t'):
