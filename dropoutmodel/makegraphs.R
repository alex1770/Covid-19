...
phebrief = rename(read_csv("PHEbriefing6.csv") %>% mutate(date=dmy(`week`)), nhs_name=Region) %>% mutate(nhs_name=replace(nhs_name,nhs_name=="Yorkshire and Humber","Yorkshire and The Humber"))
...
ggplot()+geom_line(data=phedat %>% filter(sgtf+other>=200 & nhs_name %in% common), aes(x=date,y=sgtf/(sgtf+other),colour="PHE Pillar 2 via LSHTM",group=nhs_name))+geom_line(data=mydat %>% filter(nhs_name %in% common),aes(x=date,y=sgtf,colour="Modelled from ONS survey data",group=nhs_name))+geom_line(data=phebrief %>% filter(nhs_name %in% common),aes(x=date,y=`percent_SGTF among classifiable`/100,colour="PHE briefing 6",group=nhs_name))+labs(x="Date",y="Estimated percentage B.1.1.7")+scale_y_continuous(labels=scales::percent)+geom_vline(xintercept=lastdate,linetype=3)+theme(legend.title=element_blank())+facet_wrap(~nhs_name)
ggsave("sgtf.compare3.percent.png",units="in",width=10,height=7,dpi=120)

ggplot()+geom_line(data=mydat,aes(x=date,y=sgtf,colour="Modelled from ONS survey data",group=nhs_name))+geom_line(data=phebrief,aes(x=date,y=`percent_SGTF among classifiable`/100,colour="PHE briefing 6",group=nhs_name))+labs(x="Date",y="Estimated percentage B.1.1.7")+scale_y_continuous(labels=scales::percent)+geom_vline(xintercept=lastdate,linetype=3)+theme(legend.title=element_blank())+facet_wrap(~nhs_name)
